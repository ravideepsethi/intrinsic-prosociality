#!/usr/bin/env python3
"""Collect and inspect public aggregate outputs from the elasticity module."""
...
def inspect_output(output: Path) -> dict[str, Any]:
    ...
    if success.is_file():
        ...
        return {
            ...
            "primary_elasticity": primary.get("elasticity_estimate"),
            "primary_ci95_low": primary.get("elasticity_ci95_low"),
            "primary_ci95_high": primary.get("elasticity_ci95_high"),
...
def execute(args: argparse.Namespace) -> tuple[Path, Path, dict[str, Any]]:
    ...
    zip_path = desktop / f"KINDNESS_PRICE_ELASTICITY_RESULTS_{run_id}.zip"
    ...
    with tempfile.TemporaryDirectory(prefix="kindness_price_elasticity_collection_") as temporary:
        ...
        collection = {
            "status": "KINDNESS_PRICE_ELASTICITY_COLLECTION_OK",
        ...
        collection["files_fingerprint_sha256"] = common.sha256_json(collection["files"])
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    print("\nKINDNESS PRICE ELASTICITY RESULT INSPECTION", flush=True)
    print(f"Status: {inspection.get('status')}", flush=True)
    if inspection.get("scientific_success"):
        print(f"Primary model: {inspection.get('primary_model')}", flush=True)
        print(
            "Primary elasticity / 95% CI: "
            f"{inspection.get('primary_elasticity')} / "
            f"[{inspection.get('primary_ci95_low')}, {inspection.get('primary_ci95_high')}]",
            flush=True,
        )
        print(f"Direction: {inspection.get('direction')}", flush=True)
        print(f"Claim boundary: {inspection.get('claim_boundary')}", flush=True)
        print(
            "Models attempted / estimated / failed-retained: "
            f"{inspection.get('models_attempted')} / {inspection.get('models_estimated')} / "
            f"{inspection.get('models_failed_retained')}",
            flush=True,
        )
    else:
        print(f"Failure: {inspection.get('failure_type')}: {inspection.get('failure_message')}", flush=True)
    print("\nKINDNESS_PRICE_ELASTICITY_RESULT_BUNDLE_READY", flush=True)
