        ...
        for username, role, slot in group_users:
            ...
            if role == "kind":
                if pattern == 0:
                    ...
                    # Exercise the fifth chooser-bin term without making it an
                    # exact copy of the pattern-3 indicator.
                    if group % 8 == 3:
                        middle_kind_indices = {18, 19, 20}
        ...
        output.append(
            {
                ...
                "disabled": bool(group % 37 == 0) if returned else None,
                "tos_violation": bool(group % 41 == 0) if returned else None,
                # Addendum self-test: exercise explicit missing-date indicators
                # on returned profiles. This fixture-only change does not alter
                # or stand in for the authenticated production source.
                "created_at_ms": (
                    1400000000000 + group * 1000000
                    if returned and group % 13 != 0
                    else None
                ),
